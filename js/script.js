// get the modal to show the end game message
var modale = document.getElementById('mymodale');

// get the close button in the modal
var span = document.getElementsByClassName("close")[0];

// The modal for the game over game
modelfungo = function () {
    modale.style.display = "block";
    x = document.querySelector(".gamehead");
    x.textContent = "Game Over"

}
// The modal for the winning game
modelfunwin = function () {
    modale.style.display = "block";
    x = document.querySelector(".gamehead");
    x.textContent = "Congrats! You Win"

}

// the action when you click on try again of the modal
document.getElementById("demo").addEventListener("click", myFunction);
function myFunction() {
    document.location.reload();
}

// When you click on the close button to see the ending game status
span.onclick = function () {
    modale.style.display = "none";
}

// when you click arroud after the ending game to discard the modal
window.onclick = function (event) {
    if (event.target === modale) {
        modale.style.display = "none";
    }
}

// Get the current time and extract the minutes seconds to show it on the time text, if the time is over show game over and show the ending modal, and end the game.
function startTimer(duration, display) {
    var start = Date.now(), diff, minutes, seconds;

    function timer() {
        if (playing) {
            diff = duration - (((Date.now() - start) / 1000) | 0);
            minutes = (diff / 60) | 0;
            seconds = (diff % 60) | 0;
            minutes = minutes < 10 ? "0" + minutes : minutes;
            seconds = seconds < 10 ? "0" + seconds : seconds;
            display.textContent = "Time Left " + minutes + ":" + seconds;

            if (diff <= 0) {
                display.textContent = "Game Over";
                // start = Date.now() + 1000;
                playing = false
                modelfungo();
            }
        }
    }
    timer();
    setInterval(timer, 1000)
}
// parametrage du jeu : temps du jeu
window.onload = function () {
    // select the duration here
    twominutes = 30;
    x = document.querySelector("#timerel");
    startTimer(twominutes, x)
}
playing = true
// getting all the key pressed from the keyboard
window.addEventListener('keydown', doKeyDown, true);
// the function for recognizing the keys
function doKeyDown(evt) {
    var handled = false;
    if (playing) {
        switch (evt.keyCode) {
            case 38:
                m.moveup("canvas");
                handled = true
                break;
            case 40 :
                m.movedown("canvas");
                handled = true
                break;
            case 37:
                m.moveleft("canvas");
                handled = true
                break;
            case 39:
                m.moveright("canvas");
                handled = true
                break;
        }
        if (m.checker("canvas"))
            playing = false
    }
    // to prevent moving the screen, just to move the canvas
    if (handled)
        evt.preventDefault();
}


var dsd = function (size) {
    this.N = size;
    // this.P is related to the shape
    this.P = new Array(this.N);
    // this.R
    this.R = new Array(this.N);

    this.init = function () {
        for (var i = 0; i < this.N; i++) {
            this.P[i] = i;
            this.R[i] = 0;
        }
    }
    // still todo
    this.union = function (x, y) {
        var u = this.find(x);
        var v = this.find(y);
        if (this.R[u] > this.R[v]) {
            this.R[u] = this.R[v] + 1;
            this.P[u] = v;
        } else {
            this.R[v] = this.R[u] + 1;
            this.P[v] = u;
        }
    }

    // still todo
    this.find = function (x) {
        if (x === this.P[x]) {
            return this.P[x];
        }
        else {
            this.P[x] = this.find(this.P[x]);
            return this.P[x];
        }
    }
};

function StartingPosition(choices) {
    return choices[0] ;
}

var maze = function (X, Y) {
    //this.N number of rows
    this.N = X;
    // this.M number of cols
    this.M = Y;
    // this.S the size in the canva
    this.S = 25;
    // this.moves the count of the moves in the game
    this.moves = 0;
    // this.Board the representation of the board with symbols
    this.Board = new Array(2 * this.N + 1);
    // this.EL the edges in the canva
    this.EL = new Array();
    // this.vis with the values
    this.vis = new Array(2 * this.N + 1);
    this.init = function () {
        for (var i = 0; i < 2 * this.N + 1; i++) {
            this.Board[i] = new Array(2 * this.M + 1);
            this.vis[i] = new Array(2 * this.M + 1);
        }
        for ( i = 0; i < 2 * this.N + 1; i++) {
            for (var j = 0; j < 2 * this.M + 1; j++) {
                if (!(i % 2) && !(j % 2)) {
                    this.Board[i][j] = '+';
                } else if (!(i % 2)) {
                    this.Board[i][j] = '-';
                } else if (!(j % 2)) {
                    this.Board[i][j] = '|';
                } else {
                    this.Board[i][j] = ' ';
                }
                this.vis[i][j] = 0;
            }
        }
    }


    this.add_edges = function () {
        for (var i = 0; i < this.N; i++) {
            for (var j = 0; j < this.M; j++) {
                if (i !== this.N - 1) {
                    this.EL.push([[i, j], [i + 1, j], 1]);
                }
                if (j !== this.M - 1) {
                    this.EL.push([[i, j], [i, j + 1], 1]);
                }
            }
        }
    }


    this.h = function (e) {
        return e[1] * this.M + e[0];
    }
    // this will permute EL randomly
    this.randomize = function (EL) {
        for (var i = 0; i < EL.length; i++) {
            var si = Math.floor(Math.random() * 387) % EL.length;
            var tmp = EL[si];
            EL[si] = EL[i];
            EL[i] = tmp;
        }
        return EL;
    }

    // this will make the whole into the wall in the Board
    this.breakwall = function (e) {
        var x = e[0][0] + e[1][0] + 1;
        var y = e[0][1] + e[1][1] + 1;
        this.Board[x][y] = ' ';
    }

    // this todo
    this.gen_maze = function () {
        this.EL = this.randomize(this.EL);
        var D = new dsd(this.M * this.M);
        D.init();
        var s = this.h([0, 0]);
        var e = this.h([this.N - 1, this.M - 1]);
        // this.Board[1][0] = ' ';
        // todo add more than one out gate
        this.Board[2 * this.N - 1][2 * this.M] = ' ';

        for (var i = 0; i < this.EL.length; i++) {
            var x = this.h(this.EL[i][0]);
            var y = this.h(this.EL[i][1]);
            if (D.find(s) === D.find(e)) {
                if (!(D.find(x) === D.find(s) && D.find(y) === D.find(s))) {
                    if (D.find(x) !== D.find(y)) {
                        D.union(x, y);
                        this.breakwall(this.EL[i]);
                        this.EL[i][2] = 0;
                    }

                }

            } else if (D.find(x) !== D.find(y)) {
                D.union(x, y);
                this.breakwall(this.EL[i]);
                this.EL[i][2] = 0;
            }
        }

    };


    this.draw_canvas = function (id) {
        this.canvas = document.getElementById(id);
        var scale = this.S;
        temp = []
        if (this.canvas.getContext) {
            this.ctx = this.canvas.getContext('2d');
            this.Board[1][0] = '$'
            for (var i = 0; i < 2 * this.N + 1; i++) {
                for (var j = 0; j < 2 * this.M + 1; j++) {
                    if (this.Board[i][j] !== ' ') {
                        this.ctx.fillStyle = "black";
                        this.ctx.fillRect(scale * i, scale * j, scale, scale);
                    } else if (i < 2 && j < 2)
                        temp.push([i, j])
                }
            }
            x = StartingPosition(temp)
            // localizing the start
            this.Board[x[0]][x[1]] = '&'
            this.ctx.fillStyle = "#bbbbbb";
            this.ctx.fillRect(scale * x[0], scale * x[1], scale, scale);
        }
    };

    this.checkPos = function (id) {
        for (var i = 0; i < 2 * this.N + 1; i++) {
            for (var j = 0; j < 2 * this.M + 1; j++) {
                if (this.Board[i][j] === '&') {
                    return [i, j]
                }
            }
        }
    }

    this.moveclear = function (a, b) {
        var scale = this.S;
        this.ctx = this.canvas.getContext('2d');
        this.ctx.fillStyle = "#b7b7b7";
        this.ctx.fillRect(scale * a, scale * b, scale, scale);
        this.Board[a][b] = ' '
        console.log(this.Board)
    }

    this.move = function (a, b) {
        var scale = this.S;
        this.ctx = this.canvas.getContext('2d');
        this.ctx.fillStyle = "#565656";
        this.ctx.fillRect(scale * a, scale * b, scale, scale);
        this.Board[a][b] = '&'
    }

    this.moveup = function (id) {
        cord = this.checkPos(id);
        var scale = this.S;
        i = cord[0]
        j = cord[1]
        j -= 1
        if (j < 0)
            return
        else if (j > 2 * this.M)
            return
        else if (this.Board[i][j] == ' ') {
            this.moveclear(i, j + 1);
            this.move(i, j);
            this.moves += 1;
        } else
            return
    }

    this.movedown = function (id) {
        cord = this.checkPos(id);
        var scale = this.S;
        i = cord[0]
        j = cord[1]
        j += 1
        if (j < 0)
            return
        else if (j > 2 * this.M)
            return
        else if (this.Board[i][j] == ' ') {
            this.moveclear(i, j - 1);
            this.move(i, j);
            this.moves += 1;
        } else
            return
    }

    this.moveleft = function (id) {
        cord = this.checkPos(id);
        var scale = this.S;
        i = cord[0]
        j = cord[1]
        i -= 1
        if (i < 0)
            return
        else if (i > 2 * this.N)
            return
        else if (this.Board[i][j] == ' ') {
            this.moveclear(i + 1, j);
            this.move(i, j);
            this.moves += 1;
        } else
            return
    }

    this.moveright = function (id) {
        cord = this.checkPos(id);
        var scale = this.S;
        i = cord[0]
        j = cord[1]
        i += 1
        if (i < 0)
            return
        else if (i > 2 * this.N)
            return
        else if (this.Board[i][j] == ' ') {
            this.moveclear(i - 1, j);
            this.move(i, j);
            this.moves += 1;
        } else
            return
    }
    this.checker = function (id) {
        cord = this.checkPos(id);
        i = cord[0]
        j = cord[1]
        if ((i == 19 && j == 20) || (i == 1 && j == 0)) {
            modelfunwin();

            return 1
        }
        return 0
    }

    this.getMoves = function () {
        return this.moves;
    }

};

m = new maze(10, 10);
m.init();
m.add_edges();
m.gen_maze();
m.draw_canvas("canvas");

function drawMoves() {
    document.getElementById("c").innerHTML = "Moves: " + m.getMoves()
}

setInterval(drawMoves, 100);
